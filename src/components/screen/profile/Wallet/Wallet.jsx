...
import { fetchWallet, updateWallet, createWallet, deleteWallet } from '../../../../http/walletApi'
import styles from './Wallet.module.scss'

export const Wallet = ({budgetId}) => {
    ...
    useEffect(()=> {
        fetchWallet({BudgetId: budgetId}).then(data => setDataList(data))
        setDbUpdate(false)
    }, [dbUpdate])
    ...
    const submitElChange = () => {
        DataList.map(el => {
            el.status == 'update' && updateWallet({id: el.id, name: el.name})
            el.status == 'create' && createWallet({name: el.name, BudgetId: budgetId})
            el.status == 'delete' && deleteWallet(el.id)
        })
        setDbUpdate(true)
        setActive(true)
    }
    ...
    return (
        <div className={styles.budget}>
            <span className={styles.title}>Кошелек</span>
